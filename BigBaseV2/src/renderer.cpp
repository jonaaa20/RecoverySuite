...
namespace big
{
		...
		m_dxgi_swapchain(*g_pointers->m_swapchain)
	{
		...
		std::strcpy(font_cfg.Name, "Manrope");

		m_font = ImGui::GetIO().Fonts->AddFontFromMemoryTTF(const_cast<std::uint8_t*>(font_manrope), sizeof(font_manrope), 20.f, &font_cfg);
